...
const SelfAwarenessGame: React.FC<SelfAwarenessGameProps> = ({ userId }) => {
  ...
  const modules: GameModule[] = [
    {
      id: 201, // Starting from 201 to avoid conflict with other games
  ...
  const getModulesWithStatus = () => {
    return modules.map(module => ({
      ...
      locked: module.id > 201 && !userProgress.completedModules.includes(module.id - 1)
    }));
  };
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-[#1A1F2C] flex items-center gap-2">
        <Heart className="text-[#FC68B3]" />
        Self-Awareness Journey
      </h2>
      
      {/* User Progress Bar */}
      <div className="mb-8 p-4 bg-[#FC68B3]/10 rounded-lg">
      ...
      <div className="flex border-b mb-6">
        <button
          className={`py-2 px-4 font-medium ${activeTab === 'journey' ? 'text-[#FC68B3] border-b-2 border-[#FC68B3]' : 'text-gray-500'}`}
