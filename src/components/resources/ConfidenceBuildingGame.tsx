...
const ConfidenceBuildingGame: React.FC<ConfidenceBuildingGameProps> = ({ userId }) => {
  ...
  const modules: GameModule[] = [
    {
      id: 301, // Starting from 301 to avoid conflict with other games
  ...
  const getModulesWithStatus = () => {
    return modules.map(module => ({
      ...
      locked: module.id > 301 && !userProgress.completedModules.includes(module.id - 1)
    }));
  };
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-[#1A1F2C] flex items-center gap-2">
        <Star className="text-[#F5DF4D]" />
        Confidence Building Challenge
      </h2>
      
      {/* User Progress Bar */}
      <div className="mb-8 p-4 bg-[#F5DF4D]/10 rounded-lg">
      ...
      <div className="flex border-b mb-6">
        <button
          className={`py-2 px-4 font-medium ${activeTab === 'journey' ? 'text-[#F5DF4D] border-b-2 border-[#F5DF4D]' : 'text-gray-500'}`}
