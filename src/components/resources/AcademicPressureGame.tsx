...
const AcademicPressureGame: React.FC<AcademicPressureGameProps> = ({ userId }) => {
  ...
  const modules: GameModule[] = [
    {
      id: 101, // Starting from 101 to avoid conflict with other games
  ...
  const getModulesWithStatus = () => {
    return modules.map(module => ({
      ...
      locked: module.id > 101 && !userProgress.completedModules.includes(module.id - 1)
    }));
  };
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-[#1A1F2C] flex items-center gap-2">
        <BookOpen className="text-[#FF8A48]" />
        Academic Pressure Management
      </h2>
      
      {/* User Progress Bar */}
      <div className="mb-8 p-4 bg-[#FF8A48]/10 rounded-lg">
      ...
      <div className="flex border-b mb-6">
        <button
          className={`py-2 px-4 font-medium ${activeTab === 'journey' ? 'text-[#FF8A48] border-b-2 border-[#FF8A48]' : 'text-gray-500'}`}
